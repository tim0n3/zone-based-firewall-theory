...
mikrotik_zbf_infographic.html: An interactive Single Page Application (SPA) demonstrating the jump matrix logic and packet flows.
...
IP_used_on_LAN / BTH (Trusted management subnets)
...
CRITICAL: Enable Safe Mode (Click the 'Safe Mode' button in the top left, or press CTRL+X).

Copy the contents of zbf-filter.rsc and paste it into the terminal.

Verify your connection is still active and test basic connectivity.

If everything works, disable Safe Mode to commit the changes to memory.
...
Do not reboot the router manually.

Simply close Winbox, or forcefully disconnect your network cable for 15-30 seconds.

Because you used Safe Mode, RouterOS will detect the dropped management connection and automatically revert the firewall to its exact previous state.
